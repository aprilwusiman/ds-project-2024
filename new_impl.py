...
class Person(IdentifiableEntity):
    def __init__(self, id:str, name:str):
        ...
        str_name= str(name).split("(")[0] #changed

        if str_name and str_name[-1] == " ":
            self.name= str_name[0:-1] #changed
        else:
            self.name = str_name
...
class Handler:
    ...
    def setDbPathOrUrl(self, newdb_path): #changed
        if not newdb_path:
            return False  
...
class ProcessDataUploadHandler(UploadHandler): 
    ...
    def pushDataToDb(self, file):
        try:
            ...
            for table_name, columns in activity_tables.items():
                create_table_sql = f'''
...
class BasicMashup(object):
    ...
    def getCO(self, df): 
        ...
        for idx, row in df.iterrows():
                    ...
                    elif sf== "manuscript plate" or sf== "Manuscript plate": #changed
                    ...
                    elif sf== "herbarium" or sf=="Herbarium": #changed
    ...
    def getCulturalHeritageObjectsAuthoredBy(self, id):
           ...
           if len(self.metadataQuery) != 0:
               for handler in self.metadataQuery:
                     df_s=handler.getCulturalHeritageObjectsAuthoredBy(id)
                     df_list.append(df_s)

               df = pd.concat(df_list, ignore_index=True).drop_duplicates().fillna("")
...
class AdvancedMashup(BasicMashup):
    ...
    def getObjectsHandledByResponsiblePerson(self, partialName: str) -> list[CulturalHeritageObject]:
        ...
        if self.metadataQuery:
            ...
            objects = [obj for obj in object_list if obj.getId() in object_ids]
                 ...
                 if type(x) != str:
                     if str(x.getId())== str(obj.getId()):
                         ...
                         for auth in authors_list:
                              author_id= auth.getId() #changed
                              if author_id not in set_author_id:
                                   set_author_id.add(author_id)
                                   authors.append(auth)
